...
import {
  SEED_ORG_ADMIN_NOTIFICATIONS,
  type OrgAdminNotification,
  type OrgAdminNotificationKind,
} from "@/lib/orgAdminNotificationsData";
...
function iconFor(kind: OrgAdminNotificationKind) {
  switch (kind) {
    case "user_onboarded":
      return UserPlus;
    case "invoice_ready":
...
export function OrgAdminNotificationsScreen() {
  const [notifications] = useLocalStorageState<OrgAdminNotification[]>(
    StorageKeys.orgAdminNotifications,
    SEED_ORG_ADMIN_NOTIFICATIONS,
  );
    ...
    <div className="mx-auto max-w-3xl space-y-6 pt-6">
      <div>
        ...
        <p className="mt-0.5 text-caption text-muted-foreground">
          Stay informed about user activity, subscription updates, and organization-related events.
        </p>
      </div>
        ...
        <CardContent className="flex flex-col gap-3">
          {sorted.length === 0 ? (
            ...
            sorted.map((notification) => {
              ...
              const isAcknowledged = isPolicy
                ? acknowledged.includes(notification.policyId!)
                : false;
